...
$txt_src="";
$dr_affiche=ExecuteReader($cnx,"select * from Poles");
if(isset($_GET) && count($_GET)>0)
{
    ...
    {
        $dr_affiche = ExecuteReader($cnx,"select * from Poles where codePL = '$txt_src' or NomPL = '$txt_src'");
    }
...
$("#btnCher").on("click",function(){
    ...
    ({
        rules: {
            ...
            messages: {
                txt_src: {required:'veuillez insérer code Poles ou le Nom *'}
            },
    });
});
...
<div class="main">
    <form action="Cslt_Poles.php" method="get" id="frm1">
        <div class="main_1">
            <div class="main_filter">
                <div class="title_filter">Filter Poles</div>
                <div class="filter">
                    <input name="txt_src" type="text" placeholder="Tappez code ou Nom">
                    <input id="btnCher" name="chercher" type="submit" value="chercher">
                    <a href="gestion_Poles.php">New Poles</a>
                </div>
            </div>
            <br>
            <div class="main_liste">
                <div class="title_liste">Listes Des Poles</div>
                <div class="liste">
                <?php
        if(isset($dr_affiche)){
        ...
        ?>
        <table id="mytable">
        <tr><th>Code</th><th>Entreprise</th><th>Nom</th><th>secteur</th><th>ville</th><th>Site Web</th><th>Code collabo</th><th>Code collabo2</th><th class="th_action">Action</th></tr>
        <?php while($xtr=$dr_affiche->fetch()){?>
            <tr><td><?php echo($xtr['codePL']); ?></td>
            <td><?php echo($xtr['CodeEntrFK']); ?></td>
            <td><?php echo($xtr['NomPL']); ?></td>
            <td><?php echo($xtr['secteurPL']); ?></td>
            <td><?php echo($xtr['ville']); ?></td>
            <td><?php echo($xtr['siteWeb']); ?></td>
            <td><?php echo($xtr['codeDPLFK1']); ?></td>
            <td><?php echo($xtr['codeDPLFK2']); ?></td>
            <td class="action"><a href="supp_Poles.php?codePL=<?php echo($xtr['codePL']); ?>" onclick="return confirm('veuillez vraiment suprimer <?php echo($xtr['NomPL']); ?>');" value="<?php echo($xtr['codePL']); ?>" class="td_button_delete">Delete</a> | <a href="edit_poles.php?codePL=<?php echo($xtr['codePL']); ?>" class="td_button_update" value="<?php echo($xtr['codePL']); ?>">Update</a></td></tr>
